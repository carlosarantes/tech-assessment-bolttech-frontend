<template>
  ...
  <div class="max-w-md w-full space-y-8">
    ...
    <form class="mt-8 space-y-6" action="#" method="POST">
      ...
      <div v-if="registrationErrors && registrationErrors.length > 0">
        <p class="text-red-600" v-for="(error, index) in registrationErrors" v-bind:key="index">- {{ error }}</p>
      </div>
      <div>
        <button type="button" 
          @click="register"
  ...
  computed: {
    ...
    methods : {
      setRegistrationName(e) {
        this.$store.commit('setRegistrationName', e.target.value);
      },
      setRegistrationEmail(e) {
        this.$store.commit('setRegistrationEmail', e.target.value);
      },
